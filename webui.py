...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Tabs():
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        opt_vocal_root = gr.Textbox(
                            label=i18n("指定输出主人声文件夹"), value="/mnt/gpt-sovits/output/uvr5_opt"
                        )
                        opt_ins_root = gr.Textbox(
                            label=i18n("指定输出非主人声文件夹"), value="/mnt/gpt-sovits/output/uvr5_opt"
                        )
                        format0 = gr.Radio(
                            label=i18n("导出文件格式"),
        ...
        with gr.TabItem(i18n("1-前置数据集获取工具")):#提前随机切片防止uvr5爆内存->uvr5->slicer->asr->打标
            ...
            with gr.Row():
                with gr.Row():
                    slice_inp_path=gr.Textbox(label=i18n("音频自动切分输入路径，可文件可文件夹"),value="/mnt/gpt-sovits/output/uvr5_opt")
                    slice_opt_root=gr.Textbox(label=i18n("切分后的子音频的输出根目录"),value="/mnt/gpt-sovits/output/slicer_opt")
            ...
            with gr.Row():
                ...
                denoise_input_dir=gr.Textbox(label=i18n("降噪音频文件输入文件夹"),value="/mnt/gpt-sovits/output/slicer_opt")
                denoise_output_dir=gr.Textbox(label=i18n("降噪结果输出文件夹"),value="/mnt/gpt-sovits/output/denoise_opt")
                ...
                with gr.Column():
                    with gr.Row():
                        asr_inp_dir = gr.Textbox(
                            label=i18n("输入文件夹路径"),
                            value="/mnt/gpt-sovits/output/denoise_opt",
                            interactive=True,
                        )
                        asr_opt_dir = gr.Textbox(
                            label       = i18n("输出文件夹路径"),
                            value       = "/mnt/gpt-sovits/output/asr_opt",
                            interactive = True,
                        )
                    with gr.Row():
        ...
        with gr.TabItem("1.5-训练语音文本校对"):
            path_list = gr.Textbox(
                ...
                value="/mnt/gpt-sovits/output/asr_opt/denoise_opt.list",
                interactive=True,
            )
            # reBtn = gr.Button(
            #     "刷新",
        ...
        with gr.TabItem(i18n("2-GPT-SoVITS-TTS")):
            ...
            with gr.TabItem(i18n("1A-训练集格式化工具")):
                ...
                with gr.Row():
                    inp_text = gr.Textbox(label=i18n("*文本标注文件"),value=r"/mnt/gpt-sovits/output/asr_opt/denoise_opt.list",interactive=True)
